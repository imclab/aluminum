...
public:
	...
	TriFace(MeshData::Vertex p0, MeshData::Vertex p1, MeshData::Vertex p2)
	{
		vertices[0] = p0;
		vertices[1] = p1;
		vertices[2] = p2;
...
void MeshData::compress() {
	...
	if (Ni) {
		printf("cannot compress MeshData with indices\n");
		return;
	}
	if (Nv == 0) {
		printf("cannot compress MeshData with no vertices\n");
		return;
	}
	...
	for (int i=0; i<old.vertices().size(); i++) {
		...
		if (it != imap.end()) {
			...
			if (Nt) texCoord(old.texCoords()[i]);
			// store new index:
			imap[idx] = newidx;
			// use new index:
			index(newidx);
