...
Scene :: Scene(Impl * impl) : mImpl(impl) {
  ...
  for (unsigned int i=0; i<materials(); i++) {
    ...
    max = 4;
    aiGetMaterialFloatArray(mat, AI_MATKEY_COLOR_DIFFUSE, m.diffuse, &max);
    aiGetMaterialFloatArray(mat, AI_MATKEY_COLOR_AMBIENT, m.ambient, &max);
    aiGetMaterialFloatArray(mat, AI_MATKEY_COLOR_SPECULAR, m.specular, &max);
    aiGetMaterialFloatArray(mat, AI_MATKEY_COLOR_EMISSIVE, m.emissive, &max);
    aiGetMaterialFloatArray(mat, AI_MATKEY_COLOR_TRANSPARENT, m.transparent, &max);
    aiGetMaterialFloatArray(mat, AI_MATKEY_COLOR_REFLECTIVE, m.reflective, &max);
