...
namespace aluminum {
  ...
  class Easing {
	...
	switch(ease) {
	  case IN: return in(perc); break;
	  case OUT: return out(perc); break;
	  case IN_OUT: return inout(perc); break;
	  case OUT_IN: return outin(perc); break;
	  default: return perc; break;
	}
      }
  };
  ...
  class EasingPolynomial : public Easing {
    public:
      int exponent;
      EasingPolynomial(Easing::EASE _ease, int _power) : Easing(_ease) { exponent = _power; }
      virtual double in(double perc) { return power(perc, exponent); }
      virtual double out(double perc) {	return 1.0 - power(1.0 - perc, exponent); }
  ...
  class EasingSine : public Easing {
    public:
      ...
      virtual double inout(double perc) { return (-.5 * (cos(M_PI * perc) - 1)); }
  ...
  class EasingBounce : public Easing {
    public:
      ...
      EasingBounce(Easing::EASE _ease) : Easing(_ease) { initBounce(15, .7f, .5f); }
      EasingBounce(Easing::EASE _ease, int _bounce, double _decayW, double _decayH) : Easing(_ease) { initBounce(_bounce, _decayW, _decayH); }
	...
	curveWidth = 1.0 / coeff;
      ...
      virtual double in(double perc) { return 1.0 - out(1.0 - perc); }
