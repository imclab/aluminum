  ...
  public:
    ...
    void calculatePeaks(Texture& rt, int frameNum) {
      ...
      rt.bind(); {
        ...
        memcpy ( prevVals, data, 4 * rt.width * rt.height );
    ...
    void calculateGlobalPeaks(Texture& rt, int frameNum) {
      ...
      rt.bind(); {
        ...
        for (int j = 0; j < rt.height*rt.width*4; j+=4) { //assume grayscale for now
          frameContrast += data[j];              
        } 

        frameContrast /= (float)(rt.height*rt.width);
      ...
      rt.bind(); {
        ...
        for (int j = 0; j < rt.height; j++) {
          for (int i = 0; i < rt.width; i++) {
            ...
            //int curIdx = getIdxForXY(i, j, rt.width);
            //printf("calcIndx = %d, actualIdx = %d\n", curIdx, idx);
            ...
            for (int v = 0; v < 4; v++) {
              ...
              //if ( v == 0 && ((i == 0 && j == 0 ) || ( i == 1 && j == 0)) ) 
              {
                ...
                //DL
                if (i > 0 && j > 0) {
                  int chkIdx = getIdxForXY(i-1,j-1,rt.width) + v;
                  ...
                  //L
                  if (i > 0) {
                    int chkIdx = getIdxForXY(i-1,j,rt.width) + v;
                  ...
                  //U
                  if (j < rt.height - 1) {
                    int chkIdx = getIdxForXY(i,j+1,rt.width) + v;
                    int dist = abs(data[chkIdx] - data[idx]); 
                    if (dist > maxDist) {
                      maxDist = dist;
                    }
