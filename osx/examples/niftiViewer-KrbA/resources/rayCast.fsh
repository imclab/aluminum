...
uniform sampler3D brain;  // Our 3D brain volume
...
//constants
const int MAX_SAMPLES = 500;  //total samples for each ray march step
...
//     // float alpha_level = sample - (sample * outputFrag.a);
//     // outputFrag.rgb = alpha_level * vec3(sample) + outputFrag.rgb;
//     // outputFrag.a += alpha_level;
//     brainColor = texture(brain, dataPos);
//     outputFrag = vec4(sign(opacity+0.01),2*opacity/opacity,sign(3.0*opacity),opacity);
//     outputFrag = vec4(brainColor.grb,1.0);
...
//  }
  ...
  outputFrag = vec4(brainColor.rgb,1.0);
